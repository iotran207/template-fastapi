...
- 🎯 Entry points to execute your program using `python -m <template_fastapi>` or `$ template_fastapi` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/iotran207/template-fastapi/branch/main/graph/badge.svg?token=template-fastapi_token_here)](https://codecov.io/gh/iotran207/template-fastapi)
[![CI](https://github.com/iotran207/template-fastapi/actions/workflows/main.yml/badge.svg)](https://github.com/iotran207/template-fastapi/actions/workflows/main.yml)

Awesome template_fastapi created by iotran207

## Install

from source
```bash
git clone https://github.com/iotran207/template-fastapi template_fastapi
cd template_fastapi
make install
```
...
```bash
pip install template_fastapi
```

## Executing

```bash
$ template_fastapi run --port 8080
```

or

```bash
python -m template_fastapi run --port 8080
```

or

```bash
$ uvicorn template_fastapi:app
```

## CLI

```bash
❯ template_fastapi --help
Usage: template_fastapi [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ template_fastapi create-user --help
Usage: template_fastapi create-user [OPTIONS] USERNAME PASSWORD
...
```bash
template_fastapi create-user admin admin --superuser
```
...
Run with `template_fastapi run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `template_fastapi create-user`.
...
-----------------------------------------------------
template_fastapi/__init__.py              4      0   100%
template_fastapi/app.py                  16      1    94%
template_fastapi/cli.py                  21      0   100%
template_fastapi/config.py                5      0   100%
template_fastapi/db.py                   10      0   100%
template_fastapi/models/__init__.py       0      0   100%
template_fastapi/models/content.py       47      1    98%
template_fastapi/routes/__init__.py      11      0   100%
template_fastapi/routes/content.py       52     25    52%
template_fastapi/routes/security.py      15      1    93%
template_fastapi/routes/user.py          52     26    50%
template_fastapi/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in template_fastapi/default.toml.
